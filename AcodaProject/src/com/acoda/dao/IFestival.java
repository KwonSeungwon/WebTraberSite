...
public interface IFestival {
    ...
    String update_festival="update festival set f_name=?,season=?, f_start_date=?, f_end_date=?,"
          + " f_local =?, f_contents=? "
          + " where f_number=?";
    String select_festival="select (select count(*) from festival where rownum<=100) total,f_number,f_name,season,f_start_date,f_end_date,f_local,f_contents from festival where rownum<=100 order by f_number desc" ; 
    
    String search_festival="select (select count(*) from festival where rownum<=100 and f_name like '%'||?||'%' ) total,f_number,f_name,season,f_start_date,f_end_date,f_local"
